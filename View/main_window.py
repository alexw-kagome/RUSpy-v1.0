...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.lpcutoff = 3e-5
        self.hpcutoff = 1e-3
        self.freq_raw, self.data_raw = [], []
        self.scan_freqs, self.num_points, self.tot_points = 0, 0, 0
        ...
        self.stepValue.textChanged.connect(self.update_step_value)
    ...
    def save(self):
        path = os.getcwd()
        name = os.path.join(path, f'{self.exp.get_rate()}_rate_{self.stepValue.text()}_step_{self.exp.get_sweep_size()}_sweepsize.dat')
        filename, _ = QFileDialog.getSaveFileName(self, 'Save Data', name, 'Data (*.dat)')
        f = self.freq
        if filename:
            if (self.realData[0]) and (self.imagData[0]) != 0.0:
                fh = open(filename, 'w')
                fh.write('#' + str(self.exp.get_rate()))
                fh.write(' rate (or translate to points/s)' +'\n')
                fh.write('#' + str(self.stepValue.text()))
                ...
                for i in range(len(f)):
                    fh.write('%12.5f  %12.10f  %12.10f\n' % (f[i], self.realData[i], self.imagData[i]))
                fh.close()
    '''    
    def save(self):
        path = os.getcwd()
    ...
    """
    Creates an array of scan range and number of points
    """
    def scan_range(self, start, end, stepsize):
        tot_points = (end  - start) * 1000 / stepsize
        num_scan = int(tot_points / 32000 + 0.5)
        if (num_scan < 1): 
            num_scan = 1

        scan_freqs = np.zeros(num_scan + 1, dtype=int)
        num_points = np.full(num_scan, 32000, dtype=int)
        ...
        if num_scan == 1:
            ...
            for i in range(num_scan - 1):
                scan_freqs[i + 1] = start + (i + 1) * 32 * stepsize

        print(scan_freqs[-1])
        print(scan_freqs[-2])
        num_points[-1] = int((scan_freqs[-1]  - scan_freqs[-2]) * 1000 / stepsize + 0.5)

        print(num_points)
        print(scan_freqs)
        print(num_scan)
        return scan_freqs, num_points, int(tot_points)
    
    def single_sweep(self, start, end, points):
        self.set_enabled(False)
        self.current_scan_num = self.current_scan_num + 1 
        self.update_point_value(points)
        self.update_startfreq_value(start)
        self.update_stopfreq_value(end)
        self.print_params()
    ...
    """
    Checks if single sweep has ended
    """
    def check_sweep(self):
        if self.exp.get_reading() == False:
            if self.exp.get_elapsedTime() != 0:
                self.sweepTimer.stop()
                self.freq_raw = np.concatenate([self.freq_raw, self.exp.get_reim().freq[:-1] * 1000])
                self.data_raw = np.concatenate([self.data_raw, self.exp.get_reim().data[:-1]])
                self.real_raw = np.real(self.data_raw)
                self.imag_raw = np.imag(self.data_raw)
                self.realData, self.imagData, self.freq = self.interpolate_sweep(self.real_raw, self.imag_raw, self.freq_raw)
                self.plot_data()
                self.set_enabled(True)
                if (len(self.num_points) >= self.current_scan_num):
                    self.single_sweep(self.scan_freqs[self.current_scan_num - 1], self.scan_freqs[self.current_scan_num], self.num_points[self.current_scan_num - 1])
    ...
    def start_sweep(self):
        ...
        self.sweepsize = np.sum(self.num_points)
        ...
        self.single_sweep(self.scan_freqs[0], self.scan_freqs[1], self.num_points[0])
    ...
    def update_step_value(self):
        self.scan_freqs, self.num_points, self.tot_points = self.scan_range(self.startFreqValue.value(), self.stopFreqValue.value(), int(self.stepValue.text()))
        self.pointValue.setText(str(self.tot_points))

    def update_point_value(self, value):
        self.exp.set_sweep_size(value)

    def update_startfreq_value(self, value):
        self.exp.set_sweep_start(value)

    def update_stopfreq_value(self, value):
        self.exp.set_sweep_stop(value)
    ...
    def set_enabled(self, enabled):
        widgets = [self.rateValue, self.voltageValue, self.stepValue, self.startFreqValue,
